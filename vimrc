...
map <unique> <silent> <C-F8> <ESC>:MarksBrowser<CR>
map <unique> <silent> <F8> <ESC>:Tlist<CR>
...
nmap <silent> <leader>cw :botright cw 15<CR>
"Location list windows 打开
nmap <silent> <leader>lw :botright lw 15<CR>

nmap cd :lchdir %:p:h<CR>
...
"自己定义的命令 "{{{
command CCTags !ctags --c++-kinds=+p --fields=+ialS --extra=+q -R .
command RS %s/\s\+$//ge     "消除每行后面的多余的空格
command RW %s///ge         "消除文件中的^M字符
"自己定义的命令 "}}}
...
"autocmd 如果切换文件, 自动切换当前路径, 这里屏蔽掉, eclim插件的JavaImpl冲突
"if has("autocmd")
...
highlight PmenuThumb    ctermbg=White                       guibg=Yellow
"高亮 menu color "}}}
...
"gtran在线翻译插件目前支持google和bing
source ~/.vim/bundle/gtrans/conf.vim
